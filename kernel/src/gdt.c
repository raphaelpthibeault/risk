...
gdt_init(gdt_block_t *gdtb) {
    ...
    gdt_set_entry(gdtb, 1, GDT_ACCESS_EXECUTABLE, 0);                   // kernel code segment
    gdt_set_entry(gdtb, 2, GDT_ACCESS_WRITABLE, 0);                     // kernel data segment
    gdt_set_entry(gdtb, 3, GDT_ACCESS_DPL | GDT_ACCESS_EXECUTABLE, 0);  // user code segment 
    gdt_set_entry(gdtb, 4, GDT_ACCESS_DPL | GDT_ACCESS_WRITABLE, 0);    // user data segment
    gdt_set_tss(gdtb, 6);                                               // task state segment
    // set the descriptor
...
void 
gdt_set_entry(gdt_block_t *gdtb, size_t i, enum gdt_access access, enum gdt_flags flags) {
...
void 
gdt_set_tss(gdt_block_t *gdtb, size_t gdt_index) {
    size_t tss_index = 0;
